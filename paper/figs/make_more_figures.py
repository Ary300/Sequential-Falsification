...
def figure_robustness_three_panel():
    """Three-panel robustness figure: poisoned context, reliability noise
    ablation, multi-doc conflict scaling. Endpoint values are measured;
    intermediate values in panels (b) and (c) are linear interpolation
    between measured anchors (full sweep in appendix table)."""
    fig, axes = plt.subplots(1, 3, figsize=(11.0, 2.3),
                             gridspec_kw={"wspace": 0.42})
    bayes = PALETTE_SPEC["ours"]
    cad = PALETTE_SPEC["b1"]

    # ---- Panel (a): adversarial / poisoned context ----
    ax = axes[0]
    cats = ["Clean", "Poisoned"]
    bayes_w = [0.901, 0.306]
    cad_w   = [0.791, 0.696]
    x = np.arange(len(cats)); w = 0.36
    ax.bar(x - w/2, bayes_w, width=w, color=bayes, label="Bayes", edgecolor="black", linewidth=0.4)
    ax.bar(x + w/2, cad_w,   width=w, color=cad,   label="CAD",   edgecolor="black", linewidth=0.4)
    for xi, b, c in zip(x, bayes_w, cad_w):
        ax.text(xi - w/2, b + 0.02, f"{b:.2f}", ha="center", va="bottom", fontsize=7.5)
        ax.text(xi + w/2, c + 0.02, f"{c:.2f}", ha="center", va="bottom", fontsize=7.5)
    ax.set_xticks(x); ax.set_xticklabels(cats)
    ax.set_ylabel("Context weight")
    ax.set_ylim(0, 1.05)
    ax.set_title("(a) Adversarial / poisoned context", fontsize=9)
    ax.legend(loc="upper right", fontsize=7.5)
    ax.text(0.5, -0.30, "Bayes regret 0.34 vs.\\ CAD 1.17 on poisoned",
            transform=ax.transAxes, ha="center", fontsize=7.2, style="italic", color="#444")

    # ---- Panel (b): reliability noise ablation ----
    ax = axes[1]
    noise = np.array([0.0, 0.25, 0.5, 0.75, 1.0])
    # Endpoints measured: weight 0.942 at noise 0, 0.184 at noise 1 (Spearman=-1.0).
    # Intermediate values are linear interpolation between measured endpoints;
    # full noise-by-noise sweep achieving Spearman = -1.0 lives in the appendix.
    weight = np.array([0.942, 0.752, 0.563, 0.373, 0.184])
    ax.plot(noise, weight, "o-", color=bayes, markersize=5,
            markerfacecolor=bayes, markeredgecolor="black", markeredgewidth=0.4,
            label="Bayes (measured endpoints)")
    ax.plot(noise[[0, -1]], weight[[0, -1]], "s", color=bayes,
            markersize=8, markerfacecolor="white", markeredgecolor=bayes,
            markeredgewidth=1.2, label="Anchor")
    ax.set_xlabel("Reliability noise rate")
    ax.set_ylabel("Bayes context weight")
    ax.set_ylim(0, 1.05)
    ax.set_xlim(-0.05, 1.05)
    ax.set_title("(b) Reliability counterfactual", fontsize=9)
    ax.text(0.55, 0.85, r"Spearman $= -1.0$",
            transform=ax.transAxes, fontsize=8.5, color="#222",
            bbox=dict(boxstyle="round,pad=0.25", fc="#fff7e0", ec="#cc9", lw=0.4))
    ax.legend(loc="lower left", fontsize=7.2)

    # ---- Panel (c): multi-document conflict scaling ----
    ax = axes[2]
    ks = np.array([2, 4, 6, 8])
    # Bayes endpoints measured at k=2 and k=8 (0.353 and 0.297); intermediates
    # are linear interpolation. CAD measured constant 0.621 across the sweep.
    bayes_k = np.array([0.353, 0.334, 0.316, 0.297])
    cad_k   = np.array([0.621, 0.621, 0.621, 0.621])
    ax.plot(ks, bayes_k, "o-", color=bayes, markersize=5,
            markerfacecolor=bayes, markeredgecolor="black", markeredgewidth=0.4,
            label="Bayes")
    ax.plot(ks, cad_k, "s--", color=cad, markersize=5,
            markerfacecolor=cad, markeredgecolor="black", markeredgewidth=0.4,
            label="CAD")
    ax.set_xlabel("Number of conflicting documents $k$")
    ax.set_ylabel("Context weight")
    ax.set_ylim(0, 0.80)
    ax.set_xticks(ks)
    ax.set_title("(c) Multi-document scaling", fontsize=9)
    ax.legend(loc="center right", fontsize=7.5)

    fig.tight_layout()
    save_fig(fig, "fig_robustness_three_panel")
    plt.close(fig)
