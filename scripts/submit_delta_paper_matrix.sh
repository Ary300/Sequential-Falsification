...
RESULT_PREFIX="${RESULT_PREFIX:-paperplus}"
...
RUN_MBPP="${RUN_MBPP:-0}"
RUN_LIVECODEBENCH="${RUN_LIVECODEBENCH:-1}"
...
append_model_rows() {
  ...
  if [[ "${RUN_HUMANEVAL}" == "1" ]]; then
    rows+=("${JOB_PREFIX}paper_${short}_he|${model}|humaneval_plus|results/${RESULT_PREFIX}_${short}_humaneval_full|${HUMANEVAL_MAX_PROBLEMS}|${gpus}|${cpus}|${wall}|${tp_size}|${max_model_len}|figures/${RESULT_PREFIX}_${short}_humaneval_full")
  fi
  if [[ "${RUN_MBPP}" == "1" ]]; then
    rows+=("${JOB_PREFIX}paper_${short}_mbpp|${model}|mbpp_plus|results/${RESULT_PREFIX}_${short}_mbpp_full|${MBPP_MAX_PROBLEMS}|${gpus}|${cpus}|${wall}|${tp_size}|${max_model_len}|figures/${RESULT_PREFIX}_${short}_mbpp_full")
  fi
  if [[ "${RUN_LIVECODEBENCH}" == "1" ]]; then
    rows+=("${JOB_PREFIX}paper_${short}_lcb|${model}|livecodebench_v6|results/${RESULT_PREFIX}_${short}_livecodebench_full|${LCB_MAX_PROBLEMS}|${gpus}|${cpus}|${wall}|${tp_size}|${max_model_len}|figures/${RESULT_PREFIX}_${short}_livecodebench_full")
  fi
  if [[ "${RUN_MATH500}" == "1" ]]; then
    rows+=("${JOB_PREFIX}paper_${short}_math|${model}|math500|results/${RESULT_PREFIX}_${short}_math500_full|${MATH_MAX_PROBLEMS}|${gpus}|${cpus}|${wall}|${tp_size}|${max_model_len}|figures/${RESULT_PREFIX}_${short}_math500_full")
  fi
}
