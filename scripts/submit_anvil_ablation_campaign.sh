...
PY
  ...
  IFS=$'\t' read -r exp_name benchmarks methods model backend n_candidates n_rounds temperature max_tokens probe_strategy adaptive_probe_selection eliminate_on_detection confidence_mode max_problems data_root <<< "${parsed_fields}"
