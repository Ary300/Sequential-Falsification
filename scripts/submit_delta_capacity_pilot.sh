...
sbatch --parsable --account="${GPU_ACCOUNT}" --partition="${GPU_PARTITION}" --qos="${GPU_QOS}" \
  ...
  --export=ALL,DELTA_PROJECT_ROOT=${DELTA_PROJECT_ROOT},DELTA_VENV=${DELTA_VENV},MODEL=${MODEL},BENCHMARKS=${BENCHMARKS},VERIFIERS=${VERIFIERS},OUTPUT_DIR=${OUTPUT_DIR},BACKEND=${BACKEND},SEED=${SEED},N_CANDIDATES=${N_CANDIDATES},MAX_PROBLEMS=${MAX_PROBLEMS},TIMEOUT=${TIMEOUT},TEMPERATURE=${TEMPERATURE},MAX_TOKENS=${MAX_TOKENS},TP_SIZE=${TP_SIZE},MAX_MODEL_LEN=${MAX_MODEL_LEN} \
