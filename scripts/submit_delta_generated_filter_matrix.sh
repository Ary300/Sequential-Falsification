...
RESULT_PREFIX="${RESULT_PREFIX:-reviewer_gtf}"
JOB_PREFIX="${JOB_PREFIX:-}"

DELTA_7B_WALL="${DELTA_7B_WALL:-18:00:00}"
DELTA_14B_WALL="${DELTA_14B_WALL:-18:00:00}"
...
if [[ "${RUN_MBPP}" == "1" ]]; then
  if [[ "${ENABLE_7B}" == "1" ]]; then
    rows+=("${JOB_PREFIX}gtf_r1_7b_mbpp|deepseek-ai/DeepSeek-R1-Distill-Qwen-7B|mbpp_plus|results/${RESULT_PREFIX}_r1_7b_mbpp_full|378|1|16|${DELTA_7B_WALL}|1|16384|figures/${RESULT_PREFIX}_r1_7b_mbpp_full")
  fi
  if [[ "${ENABLE_14B}" == "1" ]]; then
    rows+=("${JOB_PREFIX}gtf_qwen14b_mbpp|Qwen/Qwen2.5-Coder-14B-Instruct|mbpp_plus|results/${RESULT_PREFIX}_qwen14b_mbpp_full|378|1|16|${DELTA_14B_WALL}|1|16384|figures/${RESULT_PREFIX}_qwen14b_mbpp_full")
  fi
  if [[ "${ENABLE_32B}" == "1" ]]; then
    rows+=("${JOB_PREFIX}gtf_r1_32b_mbpp|deepseek-ai/DeepSeek-R1-Distill-Qwen-32B|mbpp_plus|results/${RESULT_PREFIX}_r1_32b_mbpp_full|378|${DELTA_32B_GPUS}|${DELTA_32B_CPUS}|${DELTA_32B_WALL}|${DELTA_32B_TP_SIZE}|${DELTA_32B_MAX_MODEL_LEN}|figures/${RESULT_PREFIX}_r1_32b_mbpp_full")
  fi
fi

if [[ "${RUN_HUMANEVAL}" == "1" ]]; then
  if [[ "${ENABLE_7B}" == "1" ]]; then
    rows+=("${JOB_PREFIX}gtf_r1_7b_he|deepseek-ai/DeepSeek-R1-Distill-Qwen-7B|humaneval_plus|results/${RESULT_PREFIX}_r1_7b_humaneval_full|164|1|16|${DELTA_7B_WALL}|1|16384|figures/${RESULT_PREFIX}_r1_7b_humaneval_full")
  fi
  if [[ "${ENABLE_14B}" == "1" ]]; then
    rows+=("${JOB_PREFIX}gtf_qwen14b_he|Qwen/Qwen2.5-Coder-14B-Instruct|humaneval_plus|results/${RESULT_PREFIX}_qwen14b_humaneval_full|164|1|16|${DELTA_14B_WALL}|1|16384|figures/${RESULT_PREFIX}_qwen14b_humaneval_full")
  fi
  if [[ "${ENABLE_32B}" == "1" ]]; then
    rows+=("${JOB_PREFIX}gtf_r1_32b_he|deepseek-ai/DeepSeek-R1-Distill-Qwen-32B|humaneval_plus|results/${RESULT_PREFIX}_r1_32b_humaneval_full|164|${DELTA_32B_GPUS}|${DELTA_32B_CPUS}|${DELTA_32B_WALL}|${DELTA_32B_TP_SIZE}|${DELTA_32B_MAX_MODEL_LEN}|figures/${RESULT_PREFIX}_r1_32b_humaneval_full")
  fi
fi
