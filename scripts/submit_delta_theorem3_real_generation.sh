...
sbatch --parsable --account="${GPU_ACCOUNT}" --partition="${GPU_PARTITION}" --qos="${GPU_QOS}" \
  ...
  --export=ALL,DELTA_PROJECT_ROOT=${DELTA_PROJECT_ROOT},DELTA_VENV=${DELTA_VENV},OUTPUT_DIR=${OUTPUT_DIR},MODEL=${MODEL},BACKEND=${BACKEND},REQUEST_TIMEOUT=${REQUEST_TIMEOUT},WIKICONTRADICT_MAX=${WIKICONTRADICT_MAX},CONFLICTBANK_MAX=${CONFLICTBANK_MAX},CONFLICTBANK_SCREENING_POOL=${CONFLICTBANK_SCREENING_POOL},AMBIGUITY_LOW=${AMBIGUITY_LOW},AMBIGUITY_HIGH=${AMBIGUITY_HIGH},TP_SIZE=${TP_SIZE},MAX_MODEL_LEN=${MAX_MODEL_LEN} \
