...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/delta_ablations}"
FIGURE_ROOT="${FIGURE_ROOT:-figures/delta_ablations}"
GPU_ACCOUNT="${GPU_ACCOUNT:-$DELTA_ACCOUNT}"
GPU_PARTITION="${GPU_PARTITION:-$DELTA_PARTITION}"
GPU_QOS="${GPU_QOS:-$DELTA_QOS}"
CPU_ACCOUNT="${CPU_ACCOUNT:-$DELTA_ACCOUNT}"
CPU_PARTITION="${CPU_PARTITION:-ghx4-interactive}"
CPU_QOS="${CPU_QOS:-}"
BACKEND_OVERRIDE="${BACKEND_OVERRIDE:-openai}"
...
if [[ -z "${LOCAL_PYTHON_BIN}" ]]; then
  if [[ -x "${DELTA_VENV}/bin/python" ]]; then
    LOCAL_PYTHON_BIN="${DELTA_VENV}/bin/python"
  else
...
cd ${DELTA_PROJECT_ROOT} && \
unset SBATCH_ACCOUNT SBATCH_PARTITION SBATCH_QOS && \
sbatch --parsable --account="${CPU_ACCOUNT}" --partition="${CPU_PARTITION}" $(cpu_qos_arg) \
  --dependency="afterany:${exp_id}" --job-name="${job_name}_rep" \
  --export=ALL,DELTA_VENV=${DELTA_VENV},RESULTS_FILE=${run_dir}/results.json,PROGRESS_DIR=${run_dir},OUTPUT_DIR=${fig_dir} \
...
EOF
  ...
  merge_wrap="cd ${DELTA_PROJECT_ROOT} && module load python/anaconda3/2.10.0 && source ${DELTA_VENV}/bin/activate && python scripts/merge_seed_results.py"
...
unset SBATCH_ACCOUNT SBATCH_PARTITION SBATCH_QOS && \
sbatch --parsable --account="${CPU_ACCOUNT}" --partition="${CPU_PARTITION}" $(cpu_qos_arg) \
  --nodes=1 --gpus-per-node=1 --cpus-per-task=4 --time="00:30:00" \
