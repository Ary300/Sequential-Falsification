# Theorem 3 Qwen Family Sweep: Completed Read

As of `2026-04-26`, the Delta same-family Qwen theorem-3 sweep has completed
and now provides paper-usable final evidence:
...
- `32B`: completed with final rows written
...
| `Qwen2.5-32B` | `ConflictBank` | conflict | `0.9484` | `0.9307` | `0.8871` | final run; conflict remains huge even at 32B |
| `Qwen2.5-32B` | `ConflictBank` | no-conflict | `0.0977` | `0.0609` | `0.0710` | final run; still far below conflict gap |
| `Qwen2.5-32B` | `WikiContradict` | conflict | `0.0945` | `0.3520` | `0.2635` | final run; first same-family long-CoT self-correction signal |
| `Qwen2.5-32B` | `WikiContradict` | no-conflict | `0.0837` | `0.3422` | `0.3232` | final run; similar but weaker recovery in no-conflict |
...
The rows above are final means from the completed generation outputs. What is
still missing is only the optional extra cleanup step of producing a dedicated
paper-quality family-figure from these numbers.
